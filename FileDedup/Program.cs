...
class Program
{
    static void Main(string[] args)
    {
        ...
        ArgDefinition[] argDefinitions = [
            ...
            new("-RestoreFrom", 1, "-restore", "-r") { Info = "string: Start enumerating from the specific file" },
            new("-MinSize", 1, "-min", "-m") { Info = "int64: Minimal file size" },
        ...
        Regex? filterRegex = ArgParser.GetString(parseResult, "-Filter", args) is string regexStrF ?
            new(regexStrF, RegexOptions.ExplicitCapture | RegexOptions.Compiled) : null;
        Regex? skipRegex = ArgParser.GetString(parseResult, "-Skip", args) is string regexStrS ?
            new(regexStrS, RegexOptions.ExplicitCapture | RegexOptions.Compiled) : null;
        long minSize = ArgParser.GetInt64(parseResult, "-MinSize", args) ?? 1;
        string? restore = ArgParser.GetString(parseResult, "-RestoreFrom", args);
        ...
        foreach (FileInfo fi in EnumerateFiles(new(dirStr), filterRegex, skipRegex))
        {
            try
            {
                ...
                else if (file is not null)
                {
                    ...
                    if (!dupOnly)
                    {
                        Console.Out.WriteLine(Convert.ToHexString(thatHash.AsBytes()));
